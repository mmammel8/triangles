...
zobrist1 = [] #zobrist hash [lines][2]
zobrist2 = [] #zobrist hash [triangles][2]
...
def test_start():
	...
	points = [Point(92,69),Point(169,49),Point(534,57),Point(248,79),Point(409,76),Point(150,160),Point(328,148),Point(516,161),Point(107,251),Point(185,287),Point(253,256),Point(384,225),Point(488,264),Point(331,313),Point(413,335),Point(68,400),Point(156,386),Point(266,398),Point(353,405),Point(507,394)]

def init():
	"""
	precompute lists of lines, triangles
	"""
	global lines, triangles, linenum, trinum, line2triangles, line_cross, crosslines, zobrist1, zobrist2
	linect = 0
	lines = []
	line2triangles = []
	...
	for i in range(NUMPOINTS):
		for j in range(i + 1, NUMPOINTS):
			if linelegal(points[i], points[j]):
				...
				#print(linect, i, j, points[i], points[j])
				linect += 1
	...
	crosslines = [ [] for _ in range(linect) ]
	zobrist1 = [[0 for _ in range(2)] for _ in range(linect)]
	zobrist2 = [[0 for _ in range(2)] for _ in range(trict)]
	maxint = pow(2,64)	
	for l1 in range(linect):
		zobrist1[l1][0] = random.randint(0,maxint)
		zobrist1[l1][1] = random.randint(0,maxint)
		for l2 in range(linect):
			if line_cross[l1][l2]:
				crosslines[l1].append(l2)
	for t1 in range(trict):
		zobrist2[t1][0] = random.randint(0,maxint)
		zobrist2[t1][1] = random.randint(0,maxint)
...
class Board:
	...
	def make_move(self, line1):
		...
		self.trimade = False
		self.zhash ^= zobrist1[line1][self.currentPlayer-1]
		for tr in line2triangles[line1]:
			self.trifilled[tr] += 1
			if self.trifilled[tr] == 3:
				...
				self.trimade = True
				self.zhash ^= zobrist2[tr][self.currentPlayer-1]				
		self.turn += 1
	...
	def MinValue(self, alpha, beta, depth):
		...
		if nmove == 0:
			...
			for i in range(min(MAXBREADTH, len(movelist))):
				mv = movelist[i][1]
				if depth == 1:
					print("A",i, mv, movelist[i][0])
				self.aiBoard.make_move(mv) #make the move on the current board
				self.threat[depth] = self.aiBoard.trimade
				if depth < MAXDEPTH1 and (depth < MAXDEPTH2 or self.threat[depth] or (depth == MAXDEPTH2 and self.threat[depth-1])):
					#try:
					#	v = self.zobrist_dict[self.aiBoard.zhash]
					#except KeyError:
					...
					self.zobrist_dict[self.aiBoard.zhash] = v
				else:
					v = self.aiBoard.get_score1()
